...
import { Activity, Users, BarChart3, Shirt, ArrowUpDown } from 'lucide-react';
                  ...
                  <CardContent className="p-4 pt-0">
                    <div className="space-y-1">
                      {lineups.home.starters.map((p, i) => (
                        <div
                          key={i}
                          className="flex items-center gap-2 text-sm py-1 px-2 rounded-lg border border-white/5 bg-white/[0.01]"
                        >
                          <span className="w-6 text-center font-bold text-white/30 text-xs">
                            {p.number}
                          </span>
                      ...
                      <div className="mt-3 pt-3 border-t border-white/5">
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          {lineups.home.substitutes.map((p, i) => (
                            <span
                              key={i}
                              className="text-xs text-white/40 bg-white/[0.02] border border-white/5 rounded-full px-2 py-0.5"
                            >
                              {p.number}. {p.name}
                            </span>
                          ))}
                      ...
                      return (
                        <div key={i}>
                          <div className="flex items-center justify-between text-xs mb-1">
                            <span className="text-white/70 font-medium tabular-nums w-10 text-right">
                              {s.home}
                            </span>
                            <span className="text-[10px] text-white/35">{statLabel(s.type)}</span>
                            <span className="text-white/70 font-medium tabular-nums w-10">
                              {s.away}
                            </span>
                          </div>
