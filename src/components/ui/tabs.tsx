...
const tabsListVariants = cva(
  'group/tabs-list inline-grid w-fit grid-flow-col auto-cols-fr items-center rounded-none p-1 text-muted-foreground group-data-[orientation=horizontal]/tabs:h-10 group-data-[orientation=vertical]/tabs:h-fit group-data-[orientation=vertical]/tabs:grid-flow-row group-data-[orientation=vertical]/tabs:auto-rows-fr data-[variant=line]:rounded-none',
  {
    variants: {
      variant: {
        default: 'bg-[#141929] border border-white/8 shadow-[inset_0_1px_0_rgba(255,255,255,0.03)]',
...
function TabsTrigger({ className, ...props }: React.ComponentProps<typeof TabsPrimitive.Trigger>) {
  return (
    <TabsPrimitive.Trigger
      ...
      className={cn(
        "relative inline-flex min-h-8 w-full cursor-pointer select-none items-center justify-center self-stretch gap-1.5 rounded-none border border-transparent px-3 text-sm font-medium whitespace-nowrap text-white/65 transition-[color,background-color,border-color,box-shadow,transform,opacity] duration-200 group-data-[orientation=vertical]/tabs:justify-start focus-visible:border-white/20 focus-visible:ring-[3px] focus-visible:ring-white/15 focus-visible:outline-none disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
        'hover:bg-white/6 hover:text-white hover:border-white/8 hover:shadow-[0_8px_24px_rgba(0,0,0,0.18)]',
        'data-[state=active]:bg-white data-[state=active]:text-bg-dark data-[state=active]:border-white data-[state=active]:shadow-[0_10px_30px_rgba(0,0,0,0.28)]',
