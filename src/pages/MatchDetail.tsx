    ...
    return (
      <div className="text-center py-20">
        ...
        <Button asChild variant="link" className="mt-2">
          <Link to="/schedule">Quay lại lịch thi đấu</Link>
        </Button>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6 max-w-2xl mx-auto">
      ...
      <Button asChild variant="ghost" className="text-gray-400 -ml-3">
        <Link to="/schedule">
          <ArrowLeft className="w-4 h-4 mr-1" /> Lịch thi đấu
        </Link>
      </Button>
      ...
      <Card className="bg-[#141929] border-white/5">
        <CardContent className="p-6">
          <div className="flex items-center justify-center gap-2 mb-4">
            {isLive && <LiveBadge />}
            {isFinished && <Badge className="bg-white/5 text-gray-400">KẾT THÚC</Badge>}
            {match.status === 'upcoming' && <Badge className="bg-white/5 text-gray-400">SẮP DIỄN RA</Badge>}
          </div>
          ...
          <CardContent>
            ...
            <div className="mt-3 p-3 rounded-lg bg-white/[0.02] border border-white/5">
              ...
              <div className="flex justify-between text-sm">
                ...
                <span className="text-gray-500">Sau Deal</span>
            ...
            <div className="space-y-2">
              {match.predictions.map((pred: any, i: number) => (
                <div
                  key={i}
                  className={cn(
                    'flex items-center justify-between p-3 rounded-lg bg-white/[0.02] border border-white/5',
                    pred.result === 'win' && 'border-green-500/10 bg-green-500/[0.02]',
                    pred.result === 'lose' && 'border-red-500/10 bg-red-500/[0.02]',
                  )}
                >
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-sm text-gray-400">
                      <img
                        src={getFlagUrl(pred.pick === 'A' ? match.team_a_code : match.team_b_code, 40)}
                        alt=""
                    ...
                    {pred.result && (
                      <Badge className={cn(
                        'text-[10px]',
                        pred.result === 'win' && 'bg-green-500/15 text-green-400',
                        pred.result === 'lose' && 'bg-red-500/15 text-red-400',
                        pred.result === 'draw' && 'bg-gray-500/15 text-gray-400',
                      )}>
      ...
      {(!match.predictions || match.predictions.length === 0) && (
        <p className="text-center text-gray-500 text-sm py-8">
          Chưa có ai đặt cược cho trận này.
        </p>
      )}
    </div>
  )
