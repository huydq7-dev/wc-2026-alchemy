...
export default function Dashboard() {
  ...
  const handleSync = async () => {
    ...
    try {
      ...
      // Refresh matches data after sync
  ...
  const stats = {
    ...
    myBets: userPreds.length,
    pendingBets: userPreds.filter((p: any) => p.result === null).length,
  }
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <div className="relative z-10">
          ...
          <p className="text-gray-400 max-w-lg mb-6">
            Dự đoán kết quả World Cup 2026 cùng nhóm Alchemy. Đặt cược, leo hạng, nhận thưởng!
          </p>

          {nextMatch && (
            <div className="mb-6">
              <p className="text-sm text-gray-500 mb-3">TRẬN TIẾP THEO</p>
              ...
              <CountdownTimer date={nextMatch.date} time={nextMatch.time} label="Bắt đầu sau" />
            </div>
          )}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <StatCard icon={Calendar} label="Tổng trận" value={stats.totalMatches} />
        <StatCard icon={Flame} label="Đã hoàn thành" value={stats.finishedMatches} color="text-green-400" />
        <StatCard icon={Trophy} label="Đã cược" value={stats.myBets} color="text-[#F5A623]" />
        <StatCard icon={Calendar} label="Đang chờ KQ" value={stats.pendingBets} color="text-blue-400" />
      </div>
      ...
      <Card className="bg-[#141929] border-white/5">
        <CardHeader className="flex flex-row items-center justify-between pb-2">
          ...
          <Button asChild variant="link" size="sm" className="text-[#C8102E]">
            <Link to="/leaderboard">Xem tất cả <ArrowRight className="w-3 h-3 ml-1" /></Link>
          </Button>
        </CardHeader>
        <CardContent>
            ...
            {top3.map((entry: any, i: number) => (
              <div
                ...
                className={cn(
                  ...
                  i === 0 && 'bg-[#F5A623]/5 border border-[#F5A623]/10'
                )}
              >
